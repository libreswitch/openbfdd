...
#ifdef ENABLE_OVSDB
  /**
   * Gets the current session state of the remote
   *
   * @return bfd::State::Value
   */
  bfd::State::Value GetRemoteState();

  bool updateDbRequired();
  bfd::State::Value GetDbState();
  void SetDbState(bfd::State::Value state);
  bfd::State::Value GetDbRemoteState();
  void SetDbRemoteState(bfd::State::Value state);
  bfd::Diag::Value GetDbLocalDiag();
  void SetDbLocalDiag(bfd::Diag::Value diag);
  bfd::Diag::Value GetDbRemoteDiag();
  void SetDbRemoteDiag(bfd::Diag::Value diag);
#endif
...
#ifdef ENABLE_OVSDB
  // DB state variables
  bfd::State::Value db_m_sessionState;
  bfd::State::Value db_m_remoteSessionState;
  bfd::Diag::Value db_m_localDiag;
  bfd::Diag::Value db_m_remoteDiag;
#endif
