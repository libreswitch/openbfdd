...
bool Beacon::Run(const list<SockAddr> &controlPorts, const list<IpAddr> &listenAddrs)
{
  ...
  // OVSDB command processor thread
  {
    ...
    CommandProcessor *ovsProcessor = MakeOvsCommandProcessor(this);
...
bool Beacon::StartActiveSession(const IpAddr &remoteAddr, const IpAddr &localAddr)
{
  ...
  if (!bfdBackendUpdateSessionChange(session))
  {
    gLog.LogError("Unable to update new session state in DB, Session id=%u", session->GetId());
  }
#endif
