...
#ifdef ENABLE_OVSDB
   db_m_sessionState(bfd::State::Down),
   db_m_remoteSessionState(bfd::State::Down),
   db_m_localDiag(bfd::Diag::None),
   db_m_remoteDiag(bfd::Diag::None),
#endif
   m_forcedState(false),
...
bool Session::ProcessControlPacket(const BfdPacket &packet, in_port_t port)
{
  ...
  if (updateDbRequired())
  {
    // Update the state change in the DB
    if (!bfdBackendUpdateSessionChange(this)) {
        gLog.LogError("Unable to update DB with session(id=%u) state change", m_id);
    }
  }
#endif
...
#ifdef ENABLE_OVSDB
bfd::State::Value Session::GetRemoteState()
{
  LogAssert(m_scheduler->IsMainThread());
  return m_remoteSessionState;
}

bool Session::updateDbRequired()
{
  if ((db_m_remoteSessionState != m_remoteSessionState) ||
      (db_m_sessionState != m_sessionState) ||
      (db_m_localDiag != m_localDiag) ||
      (db_m_remoteDiag != m_remoteDiag))
	return true;

  return false;
}

bfd::State::Value Session::GetDbState()
{
  LogAssert(m_scheduler->IsMainThread());
  return db_m_sessionState;
}

void Session::SetDbState(bfd::State::Value state)
{
  LogAssert(m_scheduler->IsMainThread());
  db_m_sessionState = state;
}

bfd::State::Value Session::GetDbRemoteState()
{
  LogAssert(m_scheduler->IsMainThread());
  return db_m_remoteSessionState;
}

void Session::SetDbRemoteState(bfd::State::Value state)
{
  LogAssert(m_scheduler->IsMainThread());
  db_m_remoteSessionState = state;
}

bfd::Diag::Value Session::GetDbLocalDiag()
{
  LogAssert(m_scheduler->IsMainThread());
  return db_m_localDiag;
}

void Session::SetDbLocalDiag(bfd::Diag::Value diag)
{
  LogAssert(m_scheduler->IsMainThread());
  db_m_localDiag = diag;
}

bfd::Diag::Value Session::GetDbRemoteDiag()
{
  LogAssert(m_scheduler->IsMainThread());
  return db_m_remoteDiag;
}

void Session::SetDbRemoteDiag(bfd::Diag::Value diag)
{
  LogAssert(m_scheduler->IsMainThread());
  db_m_remoteDiag = diag;
}
#endif
